...
"""Example test file demonstrating testing patterns for andrewdyates.
...
class TestProjectStructure:
    """Verify andrewdyates project structure exists."""

    def test_required_directories_exist(self):
        """Ensure standard directories are present."""
        root = Path(__file__).parent.parent
        assert (root / "tests").is_dir()
        assert (root / ".AI Model").is_dir()

    def test_required_files_exist(self):
        """Ensure standard files are present."""
        root = Path(__file__).parent.parent
        assert (root / "design doc").is_file()
