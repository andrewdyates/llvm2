...
use llvm2_ir::regs::{RegClass, SpecialReg, preg_class};
...
pub enum EncodeError {
    ...
    /// `MovI` / `Movz` carries an immediate that does not fit in the 16-bit
    /// MOVZ field. This would be silently truncated by a plain `MOVZ`
    /// encoding (which is exactly how issue #366 manifested at runtime),
    /// so instead we fail the encoder with a clear error. Callers that
    /// need to materialize a wide constant must emit a MOVZ+MOVK chain in
    /// ISel, or split the MovI before the encoder sees it.
    #[error(
        "MovI/Movz immediate 0x{imm:016x} does not fit in 16 bits for {opcode:?}; \
         use a MOVZ+MOVK chain at ISel instead (#366)"
    )]
...
fn extract_base_offset(
    ...
    match inst.operands.get(base_idx) {
        Some(MachOperand::PReg(p)) => {
            let offset = if inst.operands.len() > offset_idx {
                imm_val(inst, offset_idx)
            } else {
                0
            };
            Ok((p.hw_enc() as u32, offset))
        }
...
pub fn encode_instruction(inst: &MachInst) -> Result<u32, EncodeError> {
    match inst.opcode {
        ...
        AArch64Opcode::MovI | AArch64Opcode::Movz => {
            let sf = sf_from_operand(inst, 0);
            let raw = imm_val(inst, 1);
            // The MOVZ field holds an unsigned 16-bit value, zero-extended.
            // Accept any immediate whose raw bit pattern fits in 16 bits
            // (non-negative <= 0xFFFF). Everything else is either a wide
            // constant that should have been split into MOVZ+MOVK, or a
            // negative number that cannot be represented by a single MOVZ.
            let raw_u = raw as u64;
            let fits = raw >= 0 && raw_u <= 0xFFFF;
            if !fits {
        ...
        AArch64Opcode::Movk => {
            ...
            let hw = if inst.operands.len() > 2 {
                let raw = imm_val(inst, 2);
                // Shift must be a non-negative multiple of 16 with hw = shift/16 in 0..=3.
                // (On W registers only hw ∈ {0,1} is architecturally valid; the bitfield
                // check here is the encoder-level invariant and does not depend on sf.)
                if raw < 0 || raw % 16 != 0 {
                    return Err(EncodeError::InvalidOperand {
                        opcode: inst.opcode,
                        index: 2,
                        desc: format!("MOVK shift {} must be a non-negative multiple of 16", raw),
                    });
                }
                ...
                if hw > 3 {
                    return Err(EncodeError::InvalidOperand {
                        ...
                        index: 2,
                        desc: format!(
                            "MOVK shift {} out of range (shift/16 = {} must be 0..=3)",
                            raw, hw
                        ),
                    });
                }
        ...
        AArch64Opcode::LdrRI => {
            ...
            if is_fpr(inst, 0) {
                ...
                let (size, scale) = match fp_sz {
                    FpSize::Half => (0b01, 2i64),
                    FpSize::Single => (0b10, 4i64),
                    FpSize::Double => (0b11, 8i64),
                };
                encode_load_store_auto(size, 1, 0b01, scale, offset, rn, rt)
            } else {
                let sf = sf_from_operand(inst, 0);
        ...
        AArch64Opcode::StrRI => {
            ...
            if is_fpr(inst, 0) {
                ...
                encode_load_store_auto(size, 1, 0b00, scale, offset, rn, rt)
            } else {
                let sf = sf_from_operand(inst, 0);
                let (size, scale) = if sf == 1 { (0b11, 8i64) } else { (0b10, 4i64) };
        ...
        AArch64Opcode::LdrsbRI => {
            let rt = preg_hw(inst, 0)?;
            let (rn, offset) = extract_base_offset(inst, 1, 2)?;
            encode_load_store_auto(0b00, 0, 0b11, 1, offset, rn, rt)
        }
        ...
        AArch64Opcode::LdrshRI => {
            let rt = preg_hw(inst, 0)?;
            let (rn, offset) = extract_base_offset(inst, 1, 2)?;
            encode_load_store_auto(0b01, 0, 0b11, 2, offset, rn, rt)
        }
...
mod tests {
    ...
    use llvm2_ir::operand::MachOperand;
    use llvm2_ir::regs::{
        D0, H0, PReg, S0, S1, S2, SpecialReg, V0, V1, V2, W0, W1, W2, W3, W4, W5, X0, X1, X2, X3,
        X9, X30,
    };
    ...
    fn test_all_opcodes_handled() {
        ...
        let test_cases: Vec<(AArch64Opcode, Vec<MachOperand>)> = vec![
            ...
            (AArch64Opcode::FmovFprFpr, vec![preg(V0), preg(V1)]),
    ...
    fn test_preg_hw_rejects_block_operand() {
        ...
        let inst = mk(
            ...
            vec![
                preg(X0),
                preg(X1),
                MachOperand::Block(llvm2_ir::types::BlockId(0)),
            ],
        );
        let err = preg_hw(&inst, 2);
        assert!(
    ...
    #[test]
    fn test_add_ri_shift12_imm_mask() {
        let wide = mk(
            AArch64Opcode::AddRIShift12,
        ...
        let masked = mk(
            AArch64Opcode::AddRIShift12,
            vec![preg(X0), preg(X1), imm(0x345)],
        );
        let wide_enc = encode_instruction(&wide).unwrap();
        let masked_enc = encode_instruction(&masked).unwrap();
        assert_eq!(
            wide_enc, masked_enc,
            "AddRIShift12 must mask immediates to 12 bits"
        );
