...
PURPOSE: Analyzes cyclomatic/cognitive complexity to identify problematic code.
CALLED BY: MANAGER or human (manual audits), .claude/rules/ai_template.md references it
REFERENCED: README.md, .claude/rules/ai_template.md (Useful Scripts table)
...
# Directories to skip (keep in sync with pulse.py EXCLUDE_DIRS)
SKIP_DIRS = {
    ".git",
    "__pycache__",
    "node_modules",
...
def analyze_go(root: Path, result: AnalysisResult) -> None:
    ...
    if not has_gocyclo and not has_gocognit:
        result.errors.append(
            "Go tools not installed (go install github.com/fzipp/gocyclo/cmd/gocyclo@latest)"
        )
        return
    ...
    if has_gocyclo:
        ...
        if ok:
            _process_gocyclo_output(stdout, result, lang_summary)
...
def print_summary(result: AnalysisResult) -> None:
    ...
    print("\n## Summary by Language\n", file=sys.stderr)
    print(
        f"{'Language':<12} {'Files':>6} {'SLOC':>8} {'Funcs':>6} {'AvgCC':>6} {'MaxCC':>6}",
        file=sys.stderr,
    )
